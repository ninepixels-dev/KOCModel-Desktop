...
namespace KOCModel
{
    public partial class PlantGasLines : UserControl {

        public PlantGasLines() {
            InitializeComponent();
        }
        ...
        private void runModel(object sender, EventArgs e) {
            ...
            try {
                ...
                inputFile = books.Open(Path.Combine(Environment.CurrentDirectory, @"Workbooks\gaslines_model.xlsx"));
                ...
                foreach (Excel.Range fireCell in fireBallRange)
                {
                    ...
                    InitPage.GasLineValues.plantGasLine.Add(value.ToString());

                    Marshal.FinalReleaseComObject(fireCell);
                }

                templateFile = books.Open(Path.Combine(Environment.CurrentDirectory, @"Templates\Gas Line Results.xlsx"));
